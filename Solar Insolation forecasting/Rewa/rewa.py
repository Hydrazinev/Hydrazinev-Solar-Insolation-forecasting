  ...
  input_file_path = os.path.join(base_dir,"rewa_solar_insolation_2016_2022_main_up.csv")
  ...
  df = df[(df['date_time'].dt.time >= pd.to_datetime('6:30').time()) & (df['date_time'].dt.time <= pd.to_datetime('18:00').time())]
  c1 = df.isnull().sum()
  ...
  model.add(LSTM(128, return_sequences=True, use_bias=True, activation='tanh', recurrent_activation='sigmoid'))
